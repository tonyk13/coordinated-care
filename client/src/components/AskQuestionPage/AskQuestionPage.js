		...
		try {
			...
			if (!pqNE1 && !pqNE2 && !pqHLE1 && !pqNE3 && !pqTE1 && !pqNE4) {
				...
				if (pqReputation < 50) {
					...
					try {
						const pqResponse = await axios.post(
						...
						const pqToUserResponse = await axios.post(
							`https://coordinated-care-cce88007d728.herokuapp.com/api/postQuestiontoUser/${userid}/questions`,
							pqResponse.data
						);
					} catch (error) {
						console.error("Error:", error);
		...
		<div id="askQuestionPage">
			<form id="askQuestionForm">
				<div>
					...
					<label className="askQuestionSubLabels">Limit title to 50 characters or less</label>
					<textarea
						id="pqTitle"
						maxLength="50"
				...
				<div>
					<label className="askQuestionLabels">Question Summary*</label>
					<label className="askQuestionSubLabels">Limit title to 140 characters or less</label>
					<textarea
						id="pqSummary"
						maxLength="140"
				...
				<div id="askQuestionTextDiv">
					...
					<textarea id="pqText" onChange={handlepqQuestionTextarea}></textarea>
				...
				<div>
					...
					<label className="askQuestionSubLabels">Add keywords separated by whitespace</label>
				...
				<span id="askQuestionLastLine">
					...
					/>
					<span id="askQuestionErrors">*Indicates Mandatory fields</span>
				</span>
			</form>
		</div>
	);
