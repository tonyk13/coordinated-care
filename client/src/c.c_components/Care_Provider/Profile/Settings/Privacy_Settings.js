...
export default function Privacy_Settings() {
	...
	useEffect(() => {
		...
		axios
			.get(`${baseURL}/api/employees/${employee_id}/get_privacy_settings`)
			.then((response) => {
				const {
					messagePermission,
	...
	useEffect(() => {
		...
		const employee_id = Cookies.get("employee_id");
		axios
			.put(`${baseURL}/api/employees/${employee_id}/update_privacy_settings`, { privacySettings: selections })
			.then((response) => {
