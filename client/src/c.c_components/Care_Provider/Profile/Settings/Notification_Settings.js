...
const Options = ["Text Messages", "In-App Only"];
...
export default function Notification_Settings() {
	...
	useEffect(() => {
		...
		axios
			.get(`${baseURL}/api/employees/${employee_id}/get_notification_settings`)
			.then((response) => {
				const { notificationMethods, appointmentAlerts, taskAssignmentAlerts, messageAlerts, discussionBoardAlerts } =
	...
	useEffect(() => {
		...
		const employee_id = Cookies.get("employee_id");
		axios
			.put(`${baseURL}/api/employees/${employee_id}/update_notification_settings`, { notificationSettings: selections })
			.then((response) => {
